...
#'
#' @description
#' The function creates a filename for a .csv.bz2 based on the \code{year}
#' argument in a form "accident_<year>.csv.bz2".
#' It requires a numerical or integer input otherwise ends with an error.
#' @param year Numerical or integer indicates a year of a dataset
#'
#' @return Return a character string in a format "accident_<year>.csv.bz2" that
...
fars_map_state <- function(state.num, year) {
  ...
  with(data.sub, {
    ...
    graphics::points(LONGITUD, LATITUDE, pch = 45)
  })
}
